...
def _base_canvas(top=(44, 54, 63), bottom=(20, 26, 33)) -> Image.Image:
    ...
    # Suaviza a costura entre o degradê horizontal do template (mais claro à
    # esquerda, mais escuro à direita) e o degradê vertical liso usado abaixo
    # dele, evitando uma linha visível de transição na borda do header.
    seam = 20
    band = img.crop((0, HEADER_H - seam, W, HEADER_H + seam))
    band = band.filter(ImageFilter.GaussianBlur(radius=6))
    img.paste(band, (0, HEADER_H - seam))
    return img
...
def generate_points_card(
    ...
    reward = settings.get("milestone_reward", "Cafeteira") if settings else "Cafeteira"
    ...
    if is_milestone:
        ...
        draw = ImageDraw.Draw(img)
        draw.text((CX, 1730), f"\u2615  Recompensa ao atingir a meta: {reward}",
                  font=_load_font(19, bold=True), fill=TEAL, anchor="mm")
